...
import asm.n1luik.K_multi_threading.asm.IndependenceAddSynchronized_Asm;
...
@Slf4j
public class NotErrorSafeIndependenceAddSynchronized_Asm extends ITransformer2 {
    ...
    public @NotNull Set<String> targets() {
        ...
        list2.addAll(List.of(
                //new IndependenceAddSynchronized_Asm.ReadBuf(ForgeAsm.minecraft_map.mapMethod("me/jellysquid/mods/lithium/mixin/world/player_chunk_tick/ThreadedAnvilChunkStorageMixin.m_140184_(Lnet/minecraft/server/level/ServerPlayer;)V"), null)
        ));



        File f = new File("config/K_multi_threading-mod-安全-independence-sync-Method-list.txt");
        if (f.exists()) {
                ...
                for (var methodNameInfos : list) {
                    ...
                    for (var methodNameInfo : methodNameInfos) {
                        ...
                        if (!debug_add1 && !debug_add2) {
                            log.error("Not mapping error: {} {}", methodNameInfo.name(), methodNameInfo.desc());
                        }
                    }

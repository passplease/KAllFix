...
@Slf4j
public class MixinServerChunkManager3_Asm implements ITransformer<ClassNode> {
    //@NotNull
    //@Override
...
//
        ...
        for (MethodNode method : input.methods) {
            ...
            for (AbstractInsnNode instruction : instructions) {
                if (instruction.getOpcode() == Opcodes.INVOKESTATIC && instruction instanceof MethodInsnNode methodInsnNode
                    ...
                    instructions2.add(new VarInsnNode(Opcodes.ALOAD, 0));//this
                    instructions2.add(new FieldInsnNode(Opcodes.GETFIELD, "com/ishland/c2me/opts/chunk_access/mixin/async_chunk_request/MixinServerChunkManager", strings[1], "Ljava/lang/Thread;"));//this
                }else {
                    instructions2.add(instruction);
                }
            }
    ...
    public @NotNull Set<Target> targets() {
        return Set.of(
                Target.targetClass("com/ishland/c2me/opts/chunk_access/mixin/async_chunk_request/MixinServerChunkManager"));
    }
}

...
public final class OldPrimeScalarEntitySectionMapMap<V extends EntityAccess> {
    private static final VarHandle VSIZES;
    private static final VarHandle VARRAY;
    static {

        try {
            VARRAY = MethodHandles.lookup()
                    .findVarHandle(OldPrimeScalarEntitySectionMapMap.class, "array", Entry[].class);
            VSIZES = MethodHandles.lookup()
                    .findVarHandle(OldPrimeScalarEntitySectionMapMap.class, "size", int.class);
    ...
    public static class AABBF {
        public final float minX, minY, minZ;
        public final float maxX, maxY, maxZ;
        public AABBF(float minX, float minY, float minZ,
    ...
    private static final int ClearRemoveSize = 300;
    private static final int ClearSize = 500;
    private static final int ClearMin = 10000;
    private static final int StartSize = 100;

    private static final class Entry<V extends EntityAccess> {
        long key_min;
        long key_max;
        //int[] offset;
        int offsetX;
        ...
        //short[] aabb;
        V data;
        Entry(long key_min, long key_max, V data, int offsetX, int offsetY, int offsetZ) {
    ...
    private Entry<V>[] array;
    private long global_min_key;
    private long global_max_key;
    private int size;
    private final Object lock = new Object();
    //private final Map<Integer, AABB> entity2AABBKey = new ConcurrentHashMap<>();

    @SuppressWarnings("unchecked")
    public OldPrimeScalarEntitySectionMapMap() {
    ...
    public void put(AABB key, Vec3i offset, V data) {
        synchronized (lock) {
            ...
            //aabbStorage[0] = (short) (key.minX - offset.getX());
            //aabbStorage[1] = (short) (key.minY - offset.getY());
            //aabbStorage[2] = (short) (key.minZ - offset.getZ());
            //aabbStorage[3] = (short) (key.maxX - offset.getX());
            //aabbStorage[4] = (short) (key.maxY - offset.getY());
            //aabbStorage[5] = (short) (key.maxZ - offset.getZ());
            Entry<V> newEntry = new Entry<>(km, kM, data, offset.getX(), offset.getY(), offset.getZ());
    ...
    public boolean remove(V data) {
        synchronized (lock) {
            Entry<V>[] array1 = (Entry<V>[]) VARRAY.getVolatile(this);
            if (array1.length == 0) return false;
            //long km = computeKeyMin(key.minX, key.minY, key.minZ);
            //long kM = computeKeyMax(key.maxX, key.maxY, key.maxZ);
            //short[] ta = new short[6];
            //ta[0] = (short) (key.minX - offset.getX());
            //ta[1] = (short) (key.minY - offset.getY());
            //ta[2] = (short) (key.minZ - offset.getZ());
            //ta[3] = (short) (key.maxX - offset.getX());
            //ta[4] = (short) (key.maxY - offset.getY());
            //ta[5] = (short) (key.maxZ - offset.getZ());
            int matchIdx = -1;
            for (int i = 0; i < array1.length; i++) {
                Entry<V> e = array1[i];
                //if (e.key_min != km || e.key_max != kM) continue;
                //if (e.offsetX != offset.getX() || e.offsetY != offset.getY() || e.offsetZ != offset.getZ()) continue;
                //short[] a = e.aabb;
            ...
            @SuppressWarnings("unchecked")
            Entry<V>[] newArray = (Entry<V>[]) new Entry<?>[array1.length - 1];
            if (matchIdx > 0) System.arraycopy(array1, 0, newArray, 0, matchIdx);
            if (matchIdx < array1.length - 1) System.arraycopy(array1, matchIdx + 1, newArray, matchIdx, array1.length - matchIdx - 1);
            updateGlobalKeys(newArray);
            VARRAY.setVolatile(this, newArray);
            return true;
        }
    }

    // remove all entries matching data reference. returns count removed.
    public int remove2(V data) {
        synchronized (lock) {
    ...
    private static <V extends EntityAccess> int findInsertPos(Entry<V>[] arr, long key_min) {
    ...
    public void get(AABB query, List<V> out) {
        ...
        for (int i = 0; i <= right; i++) {
            ...
            //double ax_min = (double) e.aabb[0] + e.offsetX;
            //double ay_min = (double) e.aabb[1] + e.offsetY;
            //double az_min = (double) e.aabb[2] + e.offsetZ;
            //double ax_max = (double) e.aabb[3] + e.offsetX;
            //double ay_max = (double) e.aabb[4] + e.offsetY;
            //double az_max = (double) e.aabb[5] + e.offsetZ;
    ...
    private static <V extends EntityAccess> int findRightBound(Entry<V>[] snapshot, long q_max) {

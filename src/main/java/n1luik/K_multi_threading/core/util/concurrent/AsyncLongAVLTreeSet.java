 ...
 */
public class AsyncLongAVLTreeSet extends AbstractLongSortedSet implements java.io.Serializable, Cloneable, LongSortedSet {
	 ...
	 */
	public AsyncLongAVLTreeSet(final Comparator<? super Long> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public AsyncLongAVLTreeSet(final Collection<? extends Long> c) {
		this();
		addAll(c);
	}
	 ...
	 */
	public AsyncLongAVLTreeSet(final SortedSet<Long> s) {
		this(s.comparator());
		addAll(s);
	}
	 ...
	 */
	public AsyncLongAVLTreeSet(final LongCollection c) {
		this();
		addAll(c);
	}
	 ...
	 */
	public AsyncLongAVLTreeSet(final LongSortedSet s) {
		this(s.comparator());
		addAll(s);
	}
	 ...
	 */
	public AsyncLongAVLTreeSet(final LongIterator i) {
	...
	public AsyncLongAVLTreeSet(final Iterator<?> i) {
	 ...
	 * @param c a {@link Comparator} (even better, a type-specific comparator).
	 */
	public AsyncLongAVLTreeSet(final long[] a, final int offset, final int length, final Comparator<? super Long> c) {
		this(c);
	 ...
	 * @param length the number of elements to use.
	 */
	public AsyncLongAVLTreeSet(final long[] a, final int offset, final int length) {
	 ...
	 */
	public AsyncLongAVLTreeSet(final long[] a) {
		this();
		int i = a.length;
		while (i-- != 0) add(a[i]);
	}
	 ...
	 * @param c a {@link Comparator} (even better, a type-specific comparator).
	 */
	public AsyncLongAVLTreeSet(final long[] a, final Comparator<? super Long> c) {
		this(c);
		int i = a.length;
		while (i-- != 0) add(a[i]);
	}
	...
	/**
	 * This vector remembers the path followed during the current insertion. It suffices for about
	...
	private static final class Entry implements Cloneable {
		...
		Entry next() {
			...
			if ((info & SUCC_MASK) == 0) while (next != null && (next.info & PRED_MASK) == 0) next = next.left;
			return next;
		}
	...
	*/
	...
	private class SetIterator implements LongListIterator {
		 ...
		 * meaningful when this {@link SetIterator} has been created using the nonempty constructor.
		 */
		int index = 0;
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			AsyncLongAVLTreeSet.this.remove(curr.key);
			curr = null;
		}
	}
	 ...
	 * -&infin; or &infin;). Since the subset is a view on the set, at a given moment it could happen
	...
	private final class Subset extends AbstractLongSortedSet implements java.io.Serializable, LongSortedSet {
		...
		public Subset(final long from, final boolean bottom, final long to, final boolean top) {
			if (!bottom && !top && AsyncLongAVLTreeSet.this.compare(from, to) > 0) throw new IllegalArgumentException("Start element (" + from + ") is larger than end element (" + to + ")");
			this.from = from;
			this.bottom = bottom;
			this.to = to;
			this.top = top;
		...
		final boolean in(final long k) {
			return (bottom || AsyncLongAVLTreeSet.this.compare(k, from) >= 0) && (top || AsyncLongAVLTreeSet.this.compare(k, to) < 0);
		}

		@Override
		public boolean contains(final long k) {
			return in(k) && AsyncLongAVLTreeSet.this.contains(k);
		}

		@Override
		public boolean add(final long k) {
			if (!in(k)) throw new IllegalArgumentException("Element (" + k + ") out of range [" + (bottom ? "-" : String.valueOf(from)) + ", " + (top ? "-" : String.valueOf(to)) + ")");
			return AsyncLongAVLTreeSet.this.add(k);
		}

		@Override
		public boolean remove(final long k) {
			if (!in(k)) return false;
			return AsyncLongAVLTreeSet.this.remove(k);
		}
		 ...
		 */
		public AsyncLongAVLTreeSet.Entry lastEntry() {
			if (tree == null) return null;
		...
		@Override
		public long firstLong() {
			AsyncLongAVLTreeSet.Entry e = firstEntry();
		...
		@Override
		public long lastLong() {
			AsyncLongAVLTreeSet.Entry e = lastEntry();
			if (e == null) throw new NoSuchElementException();
			return e.key;
		}
		...
		private final class SubsetIterator extends SetIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && AsyncLongAVLTreeSet.this.compare(prev.key, from) < 0) prev = null;
			}

			@Override
			void updateNext() {
				next = next.next();
				if (!top && next != null && AsyncLongAVLTreeSet.this.compare(next.key, to) >= 0) next = null;
			}
		}
	}
	...
	public Object clone() {
		AsyncLongAVLTreeSet c;
		try {
			c = (AsyncLongAVLTreeSet)super.clone();
		} catch (CloneNotSupportedException cantHappen) {
			throw new InternalError();
		}
		c.allocatePaths();

 ...
 * @see HashCommon
 */
public class VolatileLong2ObjectOpenHashMap<V> extends AbstractLong2ObjectMap<V> implements java.io.Serializable, Cloneable, Hash {
	private static final long serialVersionUID = 0L;
	private static final boolean ASSERTS = false;
	/** The array of keys. */
	protected volatile transient long[] key;
	/** The array of values. */
	protected volatile transient V[] value;
	...
	public VolatileLong2ObjectOpenHashMap(final int expected, final float f) {
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public VolatileLong2ObjectOpenHashMap(final int expected) {
	 ...
	 */
	public VolatileLong2ObjectOpenHashMap() {
	 ...
	 * @param f the load factor.
	 */
	public VolatileLong2ObjectOpenHashMap(final Map<? extends Long, ? extends V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public VolatileLong2ObjectOpenHashMap(final Map<? extends Long, ? extends V> m) {
	 ...
	 * @param f the load factor.
	 */
	public VolatileLong2ObjectOpenHashMap(final Long2ObjectMap<V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public VolatileLong2ObjectOpenHashMap(final Long2ObjectMap<V> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 */
	public VolatileLong2ObjectOpenHashMap(final long[] k, final V[] v, final float f) {
		this(k.length, f);
	 ...
	 * @throws IllegalArgumentException if {@code k} and {@code v} have different lengths.
	 */
	public VolatileLong2ObjectOpenHashMap(final long[] k, final V[] v) {
	...
	private V removeEntry(final int pos) {
		return removeEntry(pos, key, value);
	}
	private V removeEntry(final int pos, long[] key, V[] value1) {
		final V oldValue = value1[pos];
		value1[pos] = null;
		size--;
		shiftKeys(pos, key, value1);
		if (n > minN && size < maxFill / 4 && n > DEFAULT_INITIAL_SIZE) rehash(n / 2, key, value1);
		return oldValue;
	}

	private V removeNullEntry(long[] key, V[] value1) {
		containsNullKey = false;
		final V oldValue = value1[n];
		value1[n] = null;
		size--;
		if (n > minN && size < maxFill / 4 && n > DEFAULT_INITIAL_SIZE) rehash(n / 2, key, value1);
		return oldValue;
	...
	private int find(final long k, final long[] key) {
	...
	private void insert(final int pos, final long k, final V v, long[] key1, V[] value1) {
		if (pos == n) containsNullKey = true;
		key1[pos] = k;
		value1[pos] = v;
		if (size++ >= maxFill) rehash(arraySize(size + 1, f), key1, value1);
	...
	public V put(final long k, final V v) {
		long[] key1 = key;
		final int pos = find(k, key1);
		V[] value2 = value;
		if (pos < 0) {
			insert(-pos - 1, k, v, key1, value2);
			return defRetValue;
		}
		final V oldValue = value2[pos];
		value2[pos] = v;
		return oldValue;
	}
	 ...
	 */
	protected final void shiftKeys(int pos, final long[] key, V[] value1) {
	...
	public V remove(final long k) {
		V[] value1 = value;
		if (((k) == (0))) {
			if (containsNullKey) return removeNullEntry(key, value1);
			return defRetValue;
		}
		long curr;
		final long[] key = this.key;
		...
		if (((k) == (curr))) return removeEntry(pos, key, value1);
		while (true) {
	...
	public V get(final long k) {
		V[] value1 = value;
		if (((k) == (0))) return containsNullKey ? value1[n] : defRetValue;
		long curr;
		final long[] key = this.key;
		int pos;
		// The starting point.
		if (((curr = key[pos = (int)it.unimi.dsi.fastutil.HashCommon.mix((k)) & mask]) == (0))) return defRetValue;
		if (((k) == (curr))) return value1[pos];
		// There's always an unused entry.
		while (true) {
			if (((curr = key[pos = (pos + 1) & mask]) == (0))) return defRetValue;
			if (((k) == (curr))) return value1[pos];
	...
	public V getOrDefault(final long k, final V defaultValue) {
		V[] value1 = value;
		if (((k) == (0))) return containsNullKey ? value1[n] : defaultValue;
		long curr;
		final long[] key = this.key;
		int pos;
		// The starting point.
		if (((curr = key[pos = (int)it.unimi.dsi.fastutil.HashCommon.mix((k)) & mask]) == (0))) return defaultValue;
		if (((k) == (curr))) return value1[pos];
		// There's always an unused entry.
		while (true) {
	...
	public V putIfAbsent(final long k, final V v) {
		final int pos = find(k, key);
		if (pos >= 0) return value[pos];
		insert(-pos - 1, k, v, key, value);
		return defRetValue;
	}
	...
	public boolean remove(final long k, final Object v) {
		...
		final long[] key = this.key;
		if (((k) == (0))) {
			if (containsNullKey && java.util.Objects.equals(v, value1[n])) {
				removeNullEntry(key, value1);
				return true;
			}
			return false;
		}
		...
		// The starting point.
		if (((curr = key[pos = (int)it.unimi.dsi.fastutil.HashCommon.mix((k)) & mask]) == (0))) return false;
		if (((k) == (curr)) && java.util.Objects.equals(v, value1[pos])) {
			removeEntry(pos, key, value1);
			return true;
		}
		while (true) {
			if (((curr = key[pos = (pos + 1) & mask]) == (0))) return false;
	...
	public boolean replace(final long k, final V oldValue, final V v) {
		final int pos = find(k, key);
		V[] value1 = value;
		if (pos < 0 || !java.util.Objects.equals(oldValue, value1[pos])) return false;
		value1[pos] = v;
		return true;
	}
	...
	public V computeIfAbsent(final long k, final java.util.function.LongFunction<? extends V> mappingFunction) {
		...
		insert(-pos - 1, k, newValue, key1, value);
		return newValue;
	}
	...
	public V computeIfPresent(final long k, final java.util.function.BiFunction<? super Long, ? super V, ? extends V> remappingFunction) {
		...
		if (pos < 0) return defRetValue;
		V[] value1 = value;
		if (value1[pos] == null) return defRetValue;
		final V newValue = remappingFunction.apply(Long.valueOf(k), (value1[pos]));
		if (newValue == null) {
			if (((k) == (0))) removeNullEntry(key1, value1);
			else removeEntry(pos, key1, value1);
			return defRetValue;
		}
		return value1[pos] = (newValue);
	}
	...
	public V compute(final long k, final java.util.function.BiFunction<? super Long, ? super V, ? extends V> remappingFunction) {
		...
		final int pos = find(k, key1);
		V[] value1 = value;
		final V newValue = remappingFunction.apply(Long.valueOf(k), pos >= 0 ? (value1[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if (((k) == (0))) removeNullEntry(key1, value1);
				else removeEntry(pos, key1, value1);
		...
		V newVal = (newValue);
		if (pos < 0) {
			insert(-pos - 1, k, newVal, key1, value1);
			return newVal;
		}
		return value1[pos] = newVal;
	}
	...
	public V merge(final long k, final V v, final java.util.function.BiFunction<? super V, ? super V, ? extends V> remappingFunction) {
		...
		final int pos = find(k, key);
		V[] value1 = value;
		if (pos < 0 || value1[pos] == null) {
			if (pos < 0) insert(-pos - 1, k, v, key1, value1);
			else value1[pos] = v;
			return v;
		}
		final V newValue = remappingFunction.apply((value1[pos]), (v));
		if (newValue == null) {
			if (((k) == (0))) removeNullEntry(key1, value1);
			else {
				removeEntry(pos, key1, value1);
			}
			return defRetValue;
		}
		return value1[pos] = (newValue);
	...
	private abstract class MapIterator<ConsumerType> {
		...
		boolean mustReturnNullKey = VolatileLong2ObjectOpenHashMap.this.containsNullKey;
		/**
		...
		public int nextEntry() {
			...
			final long key[] = VolatileLong2ObjectOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				VolatileLong2ObjectOpenHashMap.this.remove(wrapped.getLong(-pos - 1));
	...
	private final class EntryIterator extends MapIterator<Consumer<? super Long2ObjectMap.Entry<V>>> implements ObjectIterator<Long2ObjectMap.Entry<V>> {
	...
	private abstract class MapSpliterator<ConsumerType, SplitType extends MapSpliterator<ConsumerType, SplitType>> {
		...
		boolean mustReturnNull = VolatileLong2ObjectOpenHashMap.this.containsNullKey;
	...
	private final class EntrySpliterator extends MapSpliterator<Consumer<? super Long2ObjectMap.Entry<V>>, EntrySpliterator> implements ObjectSpliterator<Long2ObjectMap.Entry<V>> {
	...
	private final class MapEntrySet extends AbstractObjectSet<Long2ObjectMap.Entry<V>> implements FastEntrySet<V> {
		...
		public boolean contains(final Object o) {
			...
			V[] value1 = value;
			if (((k) == (0))) return VolatileLong2ObjectOpenHashMap.this.containsNullKey && java.util.Objects.equals(value1[n], v);
			long curr;
			final long[] key = VolatileLong2ObjectOpenHashMap.this.key;
			int pos;
			// The starting point.
			if (((curr = key[pos = (int)it.unimi.dsi.fastutil.HashCommon.mix((k)) & mask]) == (0))) return false;
			if (((k) == (curr))) return java.util.Objects.equals(value1[pos], v);
			// There's always an unused entry.
			while (true) {
				if (((curr = key[pos = (pos + 1) & mask]) == (0))) return false;
				if (((k) == (curr))) return java.util.Objects.equals(value1[pos], v);
		...
		public boolean remove(final Object o) {
			...
			V[] value1 = value;
			if (((k) == (0))) {
				if (containsNullKey && java.util.Objects.equals(value1[n], v)) {
					removeNullEntry(key, value1);
					return true;
				}
				return false;
			...
			if (((curr = key[pos = (int)it.unimi.dsi.fastutil.HashCommon.mix((k)) & mask]) == (0))) return false;
			if (((curr) == (k))) {
				if (java.util.Objects.equals(value1[pos], v)) {
					removeEntry(pos, key, value1);
					return true;
				}
				return false;
		...
		@Override
		public void clear() {
			VolatileLong2ObjectOpenHashMap.this.clear();
		}
		...
		public void forEach(final Consumer<? super Long2ObjectMap.Entry<V>> consumer) {
			long[] key1 = key;
			V[] value1 = value;
			if (containsNullKey) consumer.accept(new AbstractLong2ObjectMap.BasicEntry<V>(key1[n], value1[n]));
			for (int pos = n; pos-- != 0;) if (!((key1[pos]) == (0))) consumer.accept(new AbstractLong2ObjectMap.BasicEntry<V>(key1[pos], value1[pos]));
		}

		/** {@inheritDoc} */
		@Override
		public void fastForEach(final Consumer<? super Long2ObjectMap.Entry<V>> consumer) {
			final FBasicEntry<V> entry = new FBasicEntry<>();
			long[] key1 = key;
			V[] value1 = value;
			if (containsNullKey) {
				entry.setKey(key1[n]);
				entry.setValue(value1[n]);
				consumer.accept(entry);
			}
			for (int pos = n; pos-- != 0;) if (!((key1[pos]) == (0))) {
				entry.setKey(key1[pos]);
				entry.setValue(value1[pos]);
				consumer.accept(entry);
			}
		}
	}
	...
	private final class KeySet extends AbstractLongSet {
		...
		public void forEach(final java.util.function.LongConsumer consumer) {
			long[] key1 = key;
			if (containsNullKey) consumer.accept(key1[n]);
			for (int pos = n; pos-- != 0;) {
				final long k = key1[pos];
		...
		public boolean remove(long k) {
			...
			VolatileLong2ObjectOpenHashMap.this.remove(k);
			return size != oldSize;
		}
	...
	public ObjectCollection<V> values() {
		if (values == null) values = new AbstractObjectCollection<V>() {
			...
			public void forEach(final Consumer<? super V> consumer) {
				V[] value1 = value;
				if (containsNullKey) consumer.accept(value1[n]);
				for (int pos = n; pos-- != 0;) if (!((key[pos]) == (0))) consumer.accept(value1[pos]);
			}
	 ...
	 */
	@SuppressWarnings("unchecked")
	protected void rehash(final int newN, final long key[], final V value[]) {
	...
	@Override
	@SuppressWarnings("unchecked")
	public VolatileLong2ObjectOpenHashMap<V> clone() {
		VolatileLong2ObjectOpenHashMap<V> c;
		try {
			c = (VolatileLong2ObjectOpenHashMap<V>)super.clone();
	...
	public int hashCode() {
		...
		for (int j = realSize(), i = 0, t = 0; j-- != 0;) {
			long[] key1 = key;
			while (((key1[i]) == (0))) i++;
			t = it.unimi.dsi.fastutil.HashCommon.long2int(key1[i]);
			if (this != value1[i]) t ^= ((value1[i]) == null ? 0 : (value1[i]).hashCode());
			h += t;
			i++;
		}
		// Zero / null keys have hash zero.
		if (containsNullKey) h += ((value1[n]) == null ? 0 : (value1[n]).hashCode());
		return h;
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		final V value[] = (V[])new Object[n + 1];
		this.key = key;
		this.value = value;
		long k;
		V v;
	...
	public static final class FBasicEntry<V> extends BasicEntry<V>{
		public FBasicEntry() {
		}

		public FBasicEntry(final Long key, final V value) {

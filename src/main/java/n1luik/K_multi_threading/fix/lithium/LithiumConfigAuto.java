package n1luik.K_multi_threading.fix.lithium;
...
public class LithiumConfigAuto extends DataCollectors.CollectTools<LithiumConfigAuto.Data>{
    public LithiumConfigAuto() {
        super("LithiumConfig", "1", Data.class);
    }

    @Override
    public boolean test(Data data) {
        Properties props = new Properties();
        if (!new File("./config/lithium.properties").isFile()) {
            return false;
        }
        try (FileInputStream fin = new FileInputStream("./config/lithium.properties")) {
            props.load(fin);
    ...
    public Data get() {
        Properties props = new Properties();
        File file = new File("./config/lithium.properties");
        if (!file.isFile()) {
            try {
